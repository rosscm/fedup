...
output:
    rmdformats::html_clean:
        ...
        lightbox: true
vignette: >
    %\VignetteIndexEntry{User guide to fedup}
...
```{r, fedupDotplot, fig.width = 9, fig.height = 9.5}
...
```{r}
pathways <- openxlsx::read.xlsx(pathwayFile)
str(pathways)
```

We see these pathways are in the form of a dataframe, with pathway IDs
stored in the `Enriched.GO.names` column and gene IDs in the `Gene.ID` column.
Now use the `readPathways` function to convert this to list format:
...
Let's use the `DNA replication` pathway as our `testGene` set and the entire
set of unique `pathways` genes as our `backgroudGene` set:

```{r}
testGene <- pathways[["DNA replication"]]
backgroundGene <- unique(as.character(unlist(pathways)))
str(testGene)
str(backgroundGene)
```

Now use `runFedup` on the data:

```{r}
fedupRes <- runFedup(testGene, backgroundGene, pathways)
print(head(fedupRes[which(fedupRes$status == "Enriched"),]))
print(head(fedupRes[which(fedupRes$status == "Depleted"),]))
```
...
```{r, fedupDotplot2, fig.width = 5, fig.height = 3.5}
fedupPlot <- fedupRes[which(fedupRes$qvalue < 0.05),]
fedupPlot$log10qvalue <- -log10(fedupPlot$qvalue + 1e-10) # log10(qvalue)
fedupPlot$pathway <- gsub("\\%.*", "", fedupPlot$pathway) # clean names
