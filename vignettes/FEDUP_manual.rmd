...
fedup_plot$log10qvalue <- -log10(fedup_plot$qvalue + 1e-10) # log10(qvalue)
fedup_plot$pathway <- gsub("\\%.*", "", fedup_plot$pathway) # clean names
p <- plotDotPlot(
  df = fedup_plot,
  x_var = "log10qvalue",
  y_var = "pathway",
...
we interpret these 76 fairly redundant pathways meaningfully? What if we could
summarize them in a way that doesn't hurt our tired brains even more? Oh I know,
let's use EnrichmentMap!
