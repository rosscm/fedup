...
#' @export
readPathways <- function(pathway_file,
                         header = TRUE,
                         pathway_col,
                         gene_col,
                         MIN_GENE = 1L,
  ...
  if (!file_type %in% supported_types) {
    ...
    if (file_type == "gmt") {
      ...
      # Ensure no empty entries present
...
#' @return GMT formatted file. Rows represent gene sets. Columns represent:
#' \itemize{
#'    \item gene set name;
#'    \item description;
...
writePathways <- function(pathways, gmt_file) {
    # Prepare pathway data table for GMT output
    tab <- data.table(
